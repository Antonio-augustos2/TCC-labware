...
function registerCandidatura($conn, $nome, $email, $id_vaga, $curriculoConteudo, $caminhoArquivo, $statusCandidato = 'Pendente') {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($curriculoConteudo !== null) {
            ...
            $sql = "INSERT INTO candidato (nome, email, status) VALUES ('$nome', '$email', '$statusCandidato')";
            if ($conn->query($sql)) {
                $id_candidato = $conn->insert_id;
                error_log("✅ Candidato criado - ID: $id_candidato");
            } else {
                error_log('❌ Erro ao inserir candidato: ' . $conn->error);
                return false;
            }
        }
    }
...
/**
 * Obter candidaturas para reprocessamento (status != Pendente e com arquivo)
 */
function getCandidaturasParaReprocessamento($conn) {
    ...
    $sql = "SELECT
            ...
            INNER JOIN candidato ca ON ca.id_candidato = c.id_candidato
            INNER JOIN vaga v ON v.id_vaga = c.id_vaga
            WHERE ca.status != 'Pendente' AND c.arquivo_path IS NOT NULL
            ORDER BY c.id_candidatura DESC";
